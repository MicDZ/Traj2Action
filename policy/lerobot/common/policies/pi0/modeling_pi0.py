...
from lerobot.common.constants import ACTION, OBS_ROBOT, ACTION_IS_PAD
...
class PI0Policy(PreTrainedPolicy):
    ...
    def select_action(self, batch: dict[str, Tensor], noise: Tensor | None = None) -> Tensor:
        ...
        # batch = {k: convert_tensor(v) if k != "state" else v for k, v in batch.items()}
    ...
    def forward(self, batch: dict[str, Tensor], noise=None, time=None) -> tuple[Tensor, dict[str, Tensor]]:
        ...
        # batch = self.normalize_inputs(batch) # state is NormalizationMode.MEAN_STD
        # batch = self.normalize_targets(batch) # actions 
        # add 'abs_actions' in batch to store absolute actions before normalization
        images, img_masks = self.prepare_images(batch)
        state = self.prepare_state(batch)
        lang_tokens, lang_masks = self.prepare_language(batch)
        ...
        # Remove padding
        # NOTE: fix bug in loss calculation, only calculate loss for the original action dim
