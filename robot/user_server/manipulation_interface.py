...
from systems.robot_manipulation_system import RobotManipulationSystem

# Import necessary functions and classes from your original script

DATA_SAVE_PATH = "./data/20250910_stack_cups"
...
def initialize_robot_system():
    ...
    try:
        if robot_system is None:
            robot_system = RobotManipulationSystem(control_type="spacemouse", save_dir=DATA_SAVE_PATH)
        else:
...
@app.route('/calibrate_cameras', methods=['POST'])
def calibrate_cameras():
    ...
    try:
        robot_system.reset_for_collection()
        logging.info("Robot reset to collection position")
        return jsonify({
            'status': 'success',
            'message': 'Robot reset to collection position'
        })
    except Exception as e:
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=5001, debug=True) # host='0.0.0.0' allows external access